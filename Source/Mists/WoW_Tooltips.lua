...
--print(ST_kodKoloru,i,ST_leftText);
               if (itemLink and (itemId == 6948)) then   -- wyjątek na Kamień Powrotu
...
function WOWSTR_onEvent(_, event, addonName)
   ...
   --QTR_PS["Test"] = Frame; -- search data
      ...
      elseif (addonName == 'Blizzard_TalentUI') then
         ST_load7 = true;
         --print("Talent_UI");
         PlayerTalentFrame:HookScript("OnShow", function() StartTicker(PlayerTalentFrame, ST_PlayerTalentFrame, 0.02) end)
...
--print("ST_MerchantFrame");
   if (TT_PS["ui1"] == "1") then
      local MercTab1 = MerchantFrameTab1Text;
      ST_CheckAndReplaceTranslationTextUI(MercTab1, true, "ui");

      local MercTab2 = MerchantFrameTab2Text;
      ST_CheckAndReplaceTranslationTextUI(MercTab2, true, "ui");

      local MercTab3 = MerchantRepairText;
      ST_CheckAndReplaceTranslationTextUI(MercTab3, true, "ui");

      local MercTab4 = MerchantPageText;
      ST_CheckAndReplaceTranslationTextUI(MercTab4, true, "ui");

    for _, region in ipairs({MerchantNextPageButton:GetRegions()}) do
        if region:GetObjectType() == "FontString" and region:GetText() == "Next" then
            local MercTab5 = region
            ST_CheckAndReplaceTranslationTextUI(MercTab5, true, "ui")
            break -- İstediğimiz metni bulduk ve değiştirdik, döngüden çıkabiliriz
        end
    end

    for _, region in ipairs({MerchantPrevPageButton:GetRegions()}) do
        if region:GetObjectType() == "FontString" and region:GetText() == "Prev" then
            local MercTab6 = region
            ST_CheckAndReplaceTranslationTextUI(MercTab6, true, "ui")
            break -- İstediğimiz metni bulduk ve değiştirdik, döngüden çıkabiliriz
        end
    end
...
-- --print(ST_MountJournal);
   -- if (TT_PS["ui4"] == "1") then
      ...
      local CJobj06 = CollectionsJournal.TitleText;
      ST_CheckAndReplaceTranslationTextUI(CJobj06, false, "ui");

      local CJobj07 = MountJournalMountButtonText;
      ST_CheckAndReplaceTranslationTextUI(CJobj07, false, "ui");

      local CJobj13 = WardrobeCollectionFrameTab1Text;
      ST_CheckAndReplaceTranslationTextUI(CJobj13, false, "ui");

      local CJobj14 = WardrobeCollectionFrameTab2Text;
      ...
      local CJobj18 = PetJournalSummonButtonText;
      ST_CheckAndReplaceTranslationTextUI(CJobj18, false, "ui");

      local CJobj19 = PetJournalFindBattleText;
      ST_CheckAndReplaceTranslationTextUI(CJobj19, false, "ui");

      local CJobj20 = PetJournal.SummonRandomPetSpellFrame.Label;
      if (WoWTR_Localization.lang == 'AR') then
         ST_CheckAndReplaceTranslationText(CJobj20, false, "ui",false,false);
      else
         ST_CheckAndReplaceTranslationTextUI(CJobj20, true, "ui");
      end

      local CJobj21 = PetJournal.HealPetSpellFrame.Label;
      if (WoWTR_Localization.lang == 'AR') then
         ST_CheckAndReplaceTranslationText(CJobj21, false, "ui",false,false);
      else
         ST_CheckAndReplaceTranslationTextUI(CJobj21, true, "ui");
      end
...
--print("ST_CharacterFrame");
   if (TT_PS["ui2"] == "1") then
      ...
      local ChFrame2 = CharacterStatsPaneCategory2NameText;   -- Attributes
      ...
      local ChFrame4 = CharacterFrameTab1Text;                       -- Tab1
      ST_CheckAndReplaceTranslationTextUI(ChFrame4, true, "ui");

      local ChFrame5 = CharacterFrameTab2Text;                       -- Tab2
      ST_CheckAndReplaceTranslationTextUI(ChFrame5, true, "ui");

      local ChFrame6 = CharacterFrameTab3Text;                       -- Tab3
      ST_CheckAndReplaceTranslationTextUI(ChFrame6, true, "ui");

      local ChFrame7 = CharacterFrameTab4Text;                       -- Tab4
      ST_CheckAndReplaceTranslationTextUI(ChFrame7, true, "ui");

      local ChFrame77 = ReputationDetailFactionDescription;          -- ReputationDetailFactionDescription
      local RDFactionName = ReputationDetailFactionName:GetText(); -- Get the Faction Name
      ST_CheckAndReplaceTranslationTextUI(ChFrame77, true, "Factions:" .. ST_RenkKoduSil(RDFactionName));
...
--print("ST_FriendsFrame");
   if (TT_PS["ui6"] == "1") then
      ...
      local Friendsobj20 = AddFriendEntryFrameTopTitle;
      ST_CheckAndReplaceTranslationTextUI(Friendsobj20, true, "ui");
      
      local Friendsobj21 = AddFriendEntryFrameOrLabel;
      ST_CheckAndReplaceTranslationTextUI(Friendsobj21, false, "ui");

      local Friendsobj22 = AddFriendEntryFrameLeftDescription;
      ST_CheckAndReplaceTranslationTextUI(Friendsobj22, false, "ui");

      local Friendsobj23 = AddFriendEntryFrameRightDescription;
      ST_CheckAndReplaceTranslationTextUI(Friendsobj23, true, "ui");

      local Friendsobj24 = AddFriendEntryFrameAcceptButtonText;
      ST_CheckAndReplaceTranslationTextUI(Friendsobj24, true, "ui");



      local Friendsobj26 = AddFriendEntryFrameCancelButtonText;
      ST_CheckAndReplaceTranslationTextUI(Friendsobj26, true, "ui");


      local Friendsobj27 = AddFriendInfoFrame
      -- AddFriendInfoFrame'in içindeki tüm bölgelerin metinlerini almak için
      local regions = {Friendsobj27:GetRegions()}
      ...
      local Friendsobj28 = AddFriendInfoFrameContinueButtonText;
...
-------------------------------------------------------------------------------------------------------
-- PLAYER TALENT FRAME
function ST_PlayerTalentFrame()
   if (TT_PS["ui1"] == "1") then
    local PlayerTF01 = PlayerTalentFrameTitleText
    ST_CheckAndReplaceTranslationTextUI(PlayerTF01, true, "ui")

    local PlayerTF02 = PlayerTalentFrameSpecializationSpellScrollFrameScrollChildDescription
    ST_CheckAndReplaceTranslationTextUI(PlayerTF02, true, "ui")

    local PlayerTF03 = PlayerTalentFrameTalents.unspentText
    ST_CheckAndReplaceTranslationTextUI(PlayerTF03, true, "ui")

    local PlayerTF04 = PlayerTalentFrameTalentsLearnButtonText
    ST_CheckAndReplaceTranslationTextUI(PlayerTF04, true, "ui")

    local PlayerTF05 = PlayerTalentFrameActivateButtonText
    ST_CheckAndReplaceTranslationTextUI(PlayerTF05, true, "ui")

    local PlayerTF06 = PlayerTalentFrameSpecializationLearnButtonText
    ST_CheckAndReplaceTranslationTextUI(PlayerTF06, true, "ui")

    local PlayerTF07 = PlayerTalentFrameTab1Text
    ST_CheckAndReplaceTranslationTextUI(PlayerTF07, true, "ui")

    local PlayerTF08 = PlayerTalentFrameTab2Text
    ST_CheckAndReplaceTranslationTextUI(PlayerTF08, true, "ui")

    local PlayerTF09 = PlayerTalentFrameTab3Text
    ST_CheckAndReplaceTranslationTextUI(PlayerTF09, true, "ui")

    local PlayerTF10 = PlayerTalentFramePetSpecializationLearnButtonText
    ST_CheckAndReplaceTranslationTextUI(PlayerTF10, true, "ui")
...
function ST_MacroFrame()
   if (TT_PS["ui1"] == "1") then
    local MacroFrame01 = MacroFrameTab1Text
    ST_CheckAndReplaceTranslationTextUI(MacroFrame01, true, "ui")
    local MacroFrame02 = MacroFrameTab2Text
...
-------------------------------------------------------------------------------------------------------
-- Guild Frame
function ST_GuildFrame() -- /run ST_GuildFrame()
    if (TT_PS["ui1"] == "1") then
        ...
        -- CommunitiesFrame
        processRegion(select(8, CommunitiesFrame:GetRegions()))
        ...
        -- CommunitiesFrame.MemberList.ColumnDisplay çocuklarını işle
        ...
        processRegion(CommunitiesFrame.MemberList.ShowOfflineButton)
...
function ST_MailFrame()
    if (TT_PS["ui1"] == "1") then
        ...
        local Mailobj26 = SendMailTitleText
        ST_CheckAndReplaceTranslationTextUI(Mailobj26, true, "ui");
    end
end
    ...
    if (TT_PS["ui1"] == "1") then
        local function CheckAndReplaceHeaderContainerTexts(headerContainers)
            ...
            local auctionFrameTexts = {
                AuctionHouseFrameSellTabText,
                AuctionHouseFrameBuyTabText,
                AuctionHouseFrameAuctionsTabText,
