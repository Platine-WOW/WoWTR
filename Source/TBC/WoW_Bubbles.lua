...
-------------------------------------------------------------------------------------------------------
BB_FallbackMode = false  -- ChatFrame_AddMessageEventFilter çalışmazsa true yapacağız
...
TT_onTutorialShow();
   ...
   -- Normal mod (ChatFrame_AddMessageEventFilter düzgün çalışıyorsa)
   if not BB_FallbackMode then
      -- Burada eski davranış korunuyor
      if ((BB_PM["chat-en"] == "1") or (BB_is_translation ~= "1")) then
         return false;     -- orijinal metni oyun yazsın
      else
         return true;      -- orijinali gizle (sadece TR görünsün)
      end

   -- Fallback mod (Blizzard filtre sistemi bozuk, eventleri biz dinliyoruz)
   else
      -- 1) ÇEVİRİ VARSA
      -- Yukarıda BB_PM["chat-tr"] == "1" ise zaten Türkçe'yi DEFAULT_CHAT_FRAME:AddMessage ile yazdırdın.
      -- Burada ekstra bir şey yapmaya gerek yok.

      -- 2) ÇEVİRİ YOKSA -> mutlaka İngilizce orijinali göster
      if (BB_is_translation ~= "1") then
         -- Burada chat-en ayarına bakmıyoruz; çeviri yoksa İngilizceyi basmak zorundayız.
         if (WoWTR_Localization.lang == 'AR') then
            DEFAULT_CHAT_FRAME:AddMessage(colorText..original_txt)
         else
            DEFAULT_CHAT_FRAME:AddMessage(
               colorText.."|cCCDDEEFF"..name_NPC..":|r "..original_txt
            )
         end
      end

      -- Fallback'te return değeri önemsiz
      return
   end
end
...
local BB_UseChatFilters = false
local BB_FilterErrorPrinted = false

local function SafeAddFilter(event, func)
    if not ChatFrame_AddMessageEventFilter then
        return false
    end

    local ok, err = pcall(ChatFrame_AddMessageEventFilter, event, func)
    if not ok then
        if not BB_FilterErrorPrinted then
            --print("|cffff0000WoWTR: ChatFrame_AddMessageEventFilter kullanılamıyor:|r", err)
            BB_FilterErrorPrinted = true
        end
        return false
    end
    return true
...
local function BB_RegisterFilters()
    local ok1 = SafeAddFilter("CHAT_MSG_MONSTER_SAY",     BB_ChatFilter)
    local ok2 = SafeAddFilter("CHAT_MSG_MONSTER_PARTY",   BB_ChatFilter)
    local ok3 = SafeAddFilter("CHAT_MSG_MONSTER_YELL",    BB_ChatFilter)
    local ok4 = SafeAddFilter("CHAT_MSG_MONSTER_WHISPER", BB_ChatFilter)
    local ok5 = SafeAddFilter("CHAT_MSG_MONSTER_EMOTE",   BB_ChatFilter)

    BB_UseChatFilters = ok1 and ok2 and ok3 and ok4 and ok5
end

local function BB_DisableMonsterMessages(frame)
    if not frame or frame:IsForbidden() then return end
    ChatFrame_RemoveMessageGroup(frame, "MONSTER_SAY")
    ChatFrame_RemoveMessageGroup(frame, "MONSTER_YELL")
    ChatFrame_RemoveMessageGroup(frame, "MONSTER_WHISPER")
    ChatFrame_RemoveMessageGroup(frame, "MONSTER_EMOTE")
end
...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        if not BB_UseChatFilters then
            BB_FallbackMode = true
            --print("|cffff0000WoWTR: Chat mesaj filtre sistemi devre dışı, fallback modda çalışıyor.|r")

            -- İngilizce satırları oyun artık yazmasın, biz kontrol edeceğiz
            BB_DisableMonsterMessages(DEFAULT_CHAT_FRAME)

            -- Eventleri kendimiz dinleyelim
            local f2 = CreateFrame("Frame")
            f2:RegisterEvent("CHAT_MSG_MONSTER_SAY")
            f2:RegisterEvent("CHAT_MSG_MONSTER_PARTY")
            f2:RegisterEvent("CHAT_MSG_MONSTER_YELL")
            f2:RegisterEvent("CHAT_MSG_MONSTER_WHISPER")
            f2:RegisterEvent("CHAT_MSG_MONSTER_EMOTE")

            f2:SetScript("OnEvent", function(self, event, msg, sender, ...)
