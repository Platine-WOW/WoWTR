...
function ST_GameTooltipOnShow()
   ...
   if not (titleObj and titleObj:GetText()) then return; end
   local titleText = titleObj:GetText();
   if (string.find(titleText, " ")) then return; end
   ...
   if itemLink then           -- items
      ...
      if itemID then
         ST_prefix = "i" .. itemID;
         if (ST_PM["item"] == "0") then return; end
      end
   elseif unitID or unitName then         -- NPCs / Units (Improved detection for TBC)
      ST_prefix = "n";        -- prefix for NPC/Unit to skip name (line 1)
   elseif spellID or (PlayerTalentFrame and PlayerTalentFrame:IsVisible()) then
      if (ClassTalentFrame and ClassTalentFrame:IsVisible() and (ClassTalentFrame:GetTab()==2)) or
         (PlayerTalentFrame and PlayerTalentFrame:IsVisible()) then
         
         -- IMPORTANT: TBC captures Talent IDs (e.g. 80, 81) which are short but unique/correct
         -- Optimizasyon: Sadece talent butonu üzerindeysek GetTalentLink çağır
         if not spellID and owner and owner.GetID and owner:GetID() > 0 then
            local oName = owner:GetName() or "";
            if string.find(oName, "Talent") then
               pcall(function()
                  local tab = PanelTemplates_GetSelectedTab(PlayerTalentFrame);
                  local index = owner:GetID();
                  if tab and index and index > 0 then
                     local link = GetTalentLink(tab, index);
                     if link then
                        local tID = string.match(link, "talent:(%d+)");
                        if tID then spellID = tID; end
                     end
                  end
               end)
            end
         ...
         if spellID then
            ST_prefix = "s" .. spellID;
            if (ST_PM["talent"] == "0") then return; end
         end
      elseif spellID then
         ST_prefix = "s" .. spellID;
         if (ST_PM["spell"] == "0") then return; end
      end
   end
   ...
   local ST_kodKoloru;
   local ST_leftText, ST_tlumaczenie, ST_hash, ST_hash2, ST_pomoc5, ST_pomoc6, ST_pomoc7;
   local _font1, _size1, _1;
   ...
   if (GameTooltip.shownMoneyFrames) then
      for i = 1, GameTooltip.shownMoneyFrames, 1 do
         ...
         _G[moneyFrameName.."PrefixText"]:SetFont(WOWTR_Font2, select(2, _G[moneyFrameName.."PrefixText"]:GetFont()));
   ...
   local ST_fromLine = (ST_prefix == "h") and 1 or 2;
   
   if (ST_TooltipsID and (ST_PM["transtitle"]=="1") and ST_TooltipsID[ST_prefix]) then
      titleObj:SetText(QTR_ExpandUnitInfo(ST_TooltipsID[ST_prefix],WOWTR_Font2).." ");
      titleObj:SetFont(WOWTR_Font2, select(2, titleObj:GetFont()));
   end
   ...
   for i = ST_fromLine, numLines, 1 do
      ...
      if (ST_leftText and (string.find(ST_leftText," ")==nil)) then
         ...
         ST_kodKoloru = OkreslKodKoloru(leftColR, leftColG, leftColB);
         if (ST_leftText and (string.len(ST_leftText)>15) and ((ST_kodKoloru == "c7") or (ST_kodKoloru == "c4") or (string.len(ST_leftText)>30))) then
            if (itemLink and (GetItemInfoInstant(itemLink) == 6948)) then
               ST_pomoc5, _ = string.find(ST_leftText,". Speak");
               ST_miasto = ST_pomoc5 and string.sub(ST_leftText,21,ST_pomoc5-1) or WoWTR_Localization.your_home;
               ST_hash = string.find(ST_leftText,' Min Cooldown)') and 1336493626 or 3076025968;
            else
               ST_hash = StringHash(ST_UsunZbedneZnaki(ST_leftText));
            end
            if (((ST_kodKoloru == "c7") or (string.len(ST_leftText)>30)) and (not ST_hash2)) then
            ...
            if string.find(ST_leftText,"<Made by") then
               ST_hash = 1381871427;
            end
            if (ST_TooltipsHS[ST_hash]) then
               if ST_hash == 1381871427 then
                  local _, endBy = string.find(ST_leftText,"<Made by ");
                  local endBy2 = string.find(ST_leftText,">");
                  local nameBy = string.sub(ST_leftText, (endBy or 0)+1, (endBy2 or 0)-1);
                  ST_tlumaczenie = ST_TooltipsHS[ST_hash];
               ...
               ST_tlumaczenie = ST_TranslatePrepare(ST_leftText, ST_tlumaczenie);
               _G["GameTooltipTextLeft"..i]:SetFont(WOWTR_Font2, select(2, _G["GameTooltipTextLeft"..i]:GetFont()));
               _G["GameTooltipTextLeft"..i]:SetText(QTR_ExpandUnitInfo(ST_tlumaczenie,false,_G["GameTooltipTextLeft"..i],WOWTR_Font2).." ");
            else
               ST_nh = 1;
   ...
   if (((ST_PM["showID"]=="1") and (string.len(ST_prefix) > 1)) or ((ST_PM["showHS"]=="1") and ST_hash2)) then
      if (numLines > 0 and ST_odstep) then GameTooltip:AddLine(" ",0,0,0); end
      local typName = " ";
      local p1 = string.sub(ST_prefix,1,1);
      if p1 == "i" then typName = "Item"; 
      elseif p1 == "s" then typName = "Spell/Talent";
      end
      local ST_ID = string.sub(ST_prefix,2);
      if ((ST_PM["showID"]=="1") and ST_ID and ST_ID ~= "h") then
         GameTooltip:AddLine(typName.." ID: "..tostring(ST_ID),0,1,1);
         local nl = GameTooltip:NumLines();
         _G["GameTooltipTextLeft"..nl]:SetFont(WOWTR_Font2, 12);
      end
      if ((ST_PM["showHS"]=="1") and ST_hash2) then
   ...
   -- Zapisywanie nieprzetłumaczonych
   if ((#ST_orygText > 0 or ST_nh == 1) and (ST_PM["saveNW"] == "1")) then
      for _, ST_origin in ipairs(ST_orygText) do
         local h = StringHash(ST_UsunZbedneZnaki(ST_origin));
         -- Meta-metinleri ve hata bildirim satırlarını filtrele
         if ST_IsValidForSave(ST_origin, h) and 
            not string.find(ST_origin, "submit a bug") and 
            not string.find(ST_origin, "F6") then
            ST_PH[h] = ST_prefix .. "@" .. ST_PrzedZapisem(ST_origin);
         end
      end
   end

   -- SON İŞLEM: Marker ekle (artık bir sonraki karede return yapacak)
   if not string.find(titleObj:GetText() or "", " ") then
      titleObj:SetText(QTR_ExpandUnitInfo(titleObj:GetText() or "", WOWTR_Font2).." ");
   end
   GameTooltip:Show();
